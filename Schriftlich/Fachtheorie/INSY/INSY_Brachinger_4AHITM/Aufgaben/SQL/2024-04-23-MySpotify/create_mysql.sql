...
create database myspotify default character set utf8 default collate utf8_general_ci;
use myspotify;

CREATE SEQUENCE seq_lied;
CREATE SEQUENCE seq_playlist INCREMENT BY 100 MINVALUE 10000;
...
CREATE TABLE Lied (
       titel VARCHAR(255),
       sid INTEGER DEFAULT NEXT VALUE FOR seq_lied,
...
CREATE TABLE Track (
	   ...
	   FOREIGN KEY (titel,sid) REFERENCES Lied(titel,sid),
       PRIMARY KEY (name, nr, type, tid)
);
...
CREATE TABLE Playlist (
       pid INTEGER DEFAULT NEXT VALUE FOR seq_playlist,
       name VARCHAR(255),
       info TEXT NOT NULL,
       email VARCHAR(255) REFERENCES Account(email),
       datum DATE,
