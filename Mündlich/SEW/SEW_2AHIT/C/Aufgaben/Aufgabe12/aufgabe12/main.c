...
void pushToLast(int number, node **head); /// I'm still figuring out the correct terminology, so function names are still a bit rough
...
{
    fflush(stdin);
    printf("Push First or Push Last (f / l)\n");
    if (getch() == 'f')
        userLoop(true);
    else
        userLoop(false);
