...
int main(int argc, char *argv[])
{
    ...
    if (fp == NULL)
    {
        printf("Error reading fil5e\n");
        exit(EXIT_FAILURE);
    }

    for(i = 0; i < 4; i++)
    {
        if(fwrite(&group[i], sizeof(struct team), 4, fp) != 4)
        {
        ...
        if(fread(&team[i], sizeof(struct team), 4, fp) != 1)
            printf("Error reading file\n");

        printf("ZEIGER: %ld", ftell(fp));

        if (fp != NULL)
            fclose(fp);

        printf("NAMR : %s zor %s\n", team[i].name, team[i].goaldifference);
    }
